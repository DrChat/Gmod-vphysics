...
void btFixedConstraint::debugDraw(btIDebugDraw *pDebugDrawer)
{
	btTransform transInA, transInB;
	transInA.setIdentity();
	transInB.setIdentity();

	transInA.setOrigin(m_rbA.getWorldTransform() * m_pivotInA);
	transInB.setOrigin(m_rbB.getWorldTransform() * m_pivotInB);
	
	// TODO: Get rotations
	
	pDebugDrawer->drawTransform(transInA, m_dbgDrawSize);
	pDebugDrawer->drawTransform(transInB, m_dbgDrawSize);
}